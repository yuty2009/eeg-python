...
class NoIncrease(object):
    """ Stops if there is no increase on a given monitor channel
    for given number of epochs.

    Parameters
    ----------
    column_name: str
        Name of column to monitor for increase.
    num_epochs: str
        Number of epochs to wait before stopping when there is no increase.
    min_increase: float, optional
        Minimum relative increase that counts as a increase. E.g. 0.1 means
        only 10% increases count as a increase and reset the counter.
    """
