...
class deepConvNet(nn.Module):
    ...
    def firstBlock(self, outF, dropoutP, kernalSize, nChan, *args, **kwargs):
        return nn.Sequential(
                ...
                Conv2dWithConstraint(25, 25, (1, nChan), padding = 0, bias= False, max_norm = 2),
                nn.BatchNorm2d(outF),
                nn.ELU(),
                nn.MaxPool2d((3, 1), stride = (3, 1))
                )
    ...
    def calculateOutSize(self, model, nTime, nChan):
        '''
        Calculate the output based on input size.
        model is from nn.Module and inputSize is a array.
        '''
        data = torch.rand(1, 1, nTime, nChan)
        model.eval()
        out = model(data).shape
        return out[2:]

    def __init__(self, nTime, nChan, nClass = 2, dropoutP = 0.25, *args, **kwargs):
        ...
        self.fSize = self.calculateOutSize(self.allButLastLayers, nTime, nChan)
        self.lastLayer = self.lastBlock(nFiltLaterLayer[-1], nClass, (self.fSize[0], 1))
...
class eegNet(nn.Module):
    def initialBlocks(self, dropoutP, *args, **kwargs):
        block1 = nn.Sequential(
                nn.Conv2d(1, self.F1, (self.C1, 1),
                          padding = (self.C1 // 2, 0), bias =False),
                nn.BatchNorm2d(self.F1),
                Conv2dWithConstraint(self.F1, self.F1 * self.D, (1, self.nChan),
                                     padding = 0, bias = False, max_norm = 1,
                                     groups=self.F1),
                nn.BatchNorm2d(self.F1 * self.D),
                nn.ELU(),
                nn.AvgPool2d((4,1), stride = (4,1)),
                nn.Dropout(p = dropoutP))
        block2 = nn.Sequential(
                nn.Conv2d(self.F1 * self.D, self.F1 * self.D,  (22, 1),
                                     padding = (22//2, 0) , bias = False,
                                     groups=self.F1* self.D),
                nn.Conv2d(self.F1 * self.D, self.F2, (1,1),
                          stride =1, bias = False, padding = 0),
                nn.BatchNorm2d(self.F2),
                nn.ELU(),
                nn.AvgPool2d((8,1), stride = (8,1)),
                nn.Dropout(p = dropoutP)
                )
    ...
    def __init__(self, nTime, nChan, nClass = 2,
        ...
        self.fSize = self.calculateOutSize(self.firstBlocks, nTime, nChan)
        self.lastLayer = self.lastBlock(self.F2, nClass, (self.fSize[0], 1))
...
class FBCNet(nn.Module):
    ...
    def SCB(self, m, nChan, nBands, doWeightNorm=True, *args, **kwargs):
        ...
        '''
        return nn.Sequential(
                Conv2dWithConstraint(nBands, m*nBands, (1, nChan), groups= nBands,
    ...
    def __init__(self, nTime, nChan, nClass = 2, nBands = 9, m = 32,
    ...
    def forward(self, x):
        ...
        x = self.scb(x)
        x = x.reshape([*x.shape[0:2], self.strideFactor, int(x.shape[2]/self.strideFactor)])
...
class FBCNet_old(nn.Module):
    ...
    def __init__(self, nTime, nChan, nClass = 2, nBands = 9, m = 4,
        ...
        self.temporalLayer = current_module.__dict__[temporalLayer](dim = 2)
